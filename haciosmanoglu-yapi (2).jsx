...
function GlobalStyle() {
  return (
    <style>{`
      @import url('https://fonts.googleapis.com/css2?family=Fraunces:ital,wght@0,600;0,900;1,600&family=Inter:wght@400;500;600;700&family=IBM+Plex+Mono:wght@400;500&display=swap');

      .hy-app { font-family: 'Inter', sans-serif; }
      .hy-display { font-family: 'Fraunces', serif; }
      ...
      {heights.map((h, i) => (
        <div
          ...
          style={{
            height: "100%",
            "--bar-scale": h,
            background: i === heights.length - 1 ? C.gold : "rgba(201,162,75,0.35)",
    ...
    <div className="hy-fade-up">
      ...
      <section className="relative flex min-h-[88vh] flex-col justify-end overflow-hidden px-5 pb-10 pt-24">
        ...
        <div className="relative z-10">
          ...
          <h1 className="hy-display text-4xl italic leading-[1.05]" style={{ color: C.porcelain }}>
        ...
        <div className="grid grid-cols-2 gap-6">
          {STATS.map((s, i) => (
            <div key={s.label} className="hy-fade-up" style={{ animationDelay: `${i * 90}ms` }}>
              <SkylineBars compact />
              <p className="hy-display mt-3 text-3xl" style={{ color: C.porcelain }}>{s.value}</p>
              <p className="mt-1 text-xs" style={{ color: "rgba(247,246,242,0.55)" }}>{s.label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="px-5 py-14" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        ...
        <h2 className="hy-display text-2xl" style={{ color: C.porcelain }}>Süreci güvenle yönetiyoruz</h2>
        <div className="mt-8 flex flex-col gap-5">
          {WHY_US.map(({ icon: Icon, title, desc }) => (
            <div key={title} className="flex gap-4">
              ...
              <div>
                <p className="font-semibold" style={{ color: C.porcelain }}>{title}</p>
                <p className="mt-1 text-sm" style={{ color: "rgba(247,246,242,0.6)" }}>{desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="px-5 py-14" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        ...
        <h2 className="hy-display text-2xl" style={{ color: C.porcelain }}>Kentsel dönüşüm adımları</h2>
        <div className="mt-8 flex flex-col">
          {PROCESS_STEPS.map((p, i) => (
            <div key={p.step} className="flex gap-4 pb-6" style={{ borderLeft: i < PROCESS_STEPS.length - 1 ? "1px solid rgba(255,255,255,0.12)" : "none", marginLeft: 15 }}>
              <div
                className="hy-mono -ml-4 flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs"
                style={{ background: C.obsidian, border: `1px solid ${C.gold}`, color: C.gold }}
              >
                {p.step}
              </div>
              <div className="-mt-1">
                <p className="font-semibold" style={{ color: C.porcelain }}>{p.title}</p>
                <p className="mt-1 text-sm" style={{ color: "rgba(247,246,242,0.6)" }}>{p.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="px-5 py-14" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        ...
        <h2 className="hy-display text-2xl" style={{ color: C.porcelain }}>Bizi ziyaret edin</h2>
        <div className="mt-6 overflow-hidden rounded-2xl" style={{ border: "1px solid rgba(255,255,255,0.1)" }}>
          <iframe
            ...
            loading="lazy"
          />
          <div className="flex items-start gap-3 p-4" style={{ background: C.graphite }}>
            <MapPin size={18} color={C.gold} className="mt-0.5 shrink-0" />
            <p className="text-sm" style={{ color: "rgba(247,246,242,0.75)" }}>{CONTACT.address}</p>
          </div>
        </div>
      </section>
      ...
      <div className="overflow-hidden rounded-2xl" style={{ border: "1px solid rgba(255,255,255,0.08)" }}>
        <div className="relative h-40 w-full">
          ...
          <span
            ...
            style={{
              background: project.status === "ongoing" ? "rgba(201,162,75,0.9)" : "rgba(247,246,242,0.9)",
              color: C.obsidian,
            }}
          >
        ...
        <div className="p-4" style={{ background: C.graphite }}>
          <p className="font-semibold" style={{ color: C.porcelain }}>{project.title}</p>
          <p className="mt-0.5 text-xs" style={{ color: "rgba(247,246,242,0.55)" }}>{project.district}</p>
...
function ProjectsTab({ initialFilter, onOpenProject }) {
  ...
  return (
    <div className="hy-fade-up px-5 pb-10 pt-24">
      <SectionEyebrow>Projeler</SectionEyebrow>
      <h2 className="hy-display text-2xl" style={{ color: C.porcelain }}>Projelerimiz</h2>
          ...
          <button
            ...
            style={
              filter === f.id
                ...
                : { border: "1px solid rgba(255,255,255,0.15)", color: "rgba(247,246,242,0.7)" }
            }
          >
            {f.label}
          </button>
    ...
    <div className="fixed inset-0 z-50 flex items-end" style={{ background: "rgba(0,0,0,0.6)" }} onClick={onClose}>
      ...
      >
        ...
        <div className="px-5 pb-8 pt-5">
          ...
          <h3 className="hy-display mt-3 text-2xl" style={{ color: C.porcelain }}>{project.title}</h3>
          <p className="mt-1 flex items-center gap-1.5 text-sm" style={{ color: "rgba(247,246,242,0.55)" }}>
          ...
          <p className="mt-5 text-sm leading-relaxed" style={{ color: "rgba(247,246,242,0.7)" }}>
...
function AboutTab() {
  return (
    <div className="hy-fade-up px-5 pb-10 pt-24">
      ...
      <h2 className="hy-display text-2xl" style={{ color: C.porcelain }}>Hikayemiz</h2>
      <p className="mt-4 text-sm leading-relaxed" style={{ color: "rgba(247,246,242,0.7)" }}>{ABOUT.history}</p>

      <div className="mt-8 grid grid-cols-1 gap-4">
        <div className="rounded-2xl p-5" style={{ background: C.graphite, border: "1px solid rgba(255,255,255,0.08)" }}>
          <p className="hy-mono text-xs uppercase tracking-[0.2em]" style={{ color: C.gold }}>Misyon</p>
          <p className="mt-2 text-sm leading-relaxed" style={{ color: "rgba(247,246,242,0.75)" }}>{ABOUT.mission}</p>
        </div>
        <div className="rounded-2xl p-5" style={{ background: C.graphite, border: "1px solid rgba(255,255,255,0.08)" }}>
          <p className="hy-mono text-xs uppercase tracking-[0.2em]" style={{ color: C.gold }}>Vizyon</p>
          <p className="mt-2 text-sm leading-relaxed" style={{ color: "rgba(247,246,242,0.75)" }}>{ABOUT.vision}</p>
        </div>
      </div>

      <h2 className="hy-display mt-10 text-2xl" style={{ color: C.porcelain }}>Değerlerimiz</h2>
      <div className="mt-6 grid grid-cols-2 gap-4">
        {ABOUT.values.map((v) => (
          <div key={v.title} className="rounded-2xl p-4" style={{ border: "1px solid rgba(255,255,255,0.08)" }}>
            <p className="font-semibold" style={{ color: C.gold }}>{v.title}</p>
            <p className="mt-1.5 text-xs leading-relaxed" style={{ color: "rgba(247,246,242,0.6)" }}>{v.desc}</p>
          </div>
        ))}
      </div>
...
function ContactTab() {
  ...
  const field = (key, label, props = {}) => (
    <div>
      <label className="mb-1.5 block text-xs font-medium" style={{ color: "rgba(247,246,242,0.6)" }}>{label}</label>
      <input
        {...props}
        value={form[key]}
        ...
        style={{
          ...
          border: `1px solid ${errors[key] ? "#c0392b" : "rgba(255,255,255,0.12)"}`,
          color: C.porcelain,
        }}
      />
  ...
  return (
    <div className="hy-fade-up px-5 pb-10 pt-24">
      ...
      <h2 className="hy-display text-2xl" style={{ color: C.porcelain }}>Bize ulaşın</h2>

      <div className="mt-6 grid grid-cols-3 gap-3">
        <a href={`tel:${CONTACT.phoneHref}`} className="flex flex-col items-center gap-1.5 rounded-xl py-3.5" style={{ border: "1px solid rgba(255,255,255,0.1)" }}>
          <Phone size={17} color={C.gold} />
          <span className="text-[11px]" style={{ color: "rgba(247,246,242,0.7)" }}>Ara</span>
        </a>
        <a href={`https://wa.me/${CONTACT.whatsapp}`} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-1.5 rounded-xl py-3.5" style={{ border: "1px solid rgba(255,255,255,0.1)" }}>
          <MessageCircle size={17} color={C.gold} />
          <span className="text-[11px]" style={{ color: "rgba(247,246,242,0.7)" }}>WhatsApp</span>
        </a>
        <a href={`mailto:${CONTACT.email}`} className="flex flex-col items-center gap-1.5 rounded-xl py-3.5" style={{ border: "1px solid rgba(255,255,255,0.1)" }}>
          <Mail size={17} color={C.gold} />
          <span className="text-[11px]" style={{ color: "rgba(247,246,242,0.7)" }}>E-posta</span>
        </a>
      </div>

      <div className="mt-8">
        {submitted ? (
          <div className="hy-fade-up flex flex-col items-center gap-3 rounded-2xl py-12 text-center" style={{ background: C.graphite }}>
            <CheckCircle2 size={32} color={C.gold} />
            <p className="font-semibold" style={{ color: C.porcelain }}>Mesajınız alındı</p>
            <p className="max-w-[240px] text-sm" style={{ color: "rgba(247,246,242,0.6)" }}>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium" style={{ color: "rgba(247,246,242,0.6)" }}>Mesaj</label>
              <textarea
                rows={4}
                value={form.message}
                ...
                style={{
                  background: C.graphite,
                  border: `1px solid ${errors.message ? "#c0392b" : "rgba(255,255,255,0.12)"}`,
                  color: C.porcelain,
                }}
              />
...
function Footer() {
  return (
    <footer className="mt-14 border-t px-1 pb-6 pt-8 text-center" style={{ borderColor: "rgba(255,255,255,0.08)" }}>
      <p className="hy-display text-lg italic" style={{ color: C.porcelain }}>Hacıosmanoğlu Yapı</p>
      <p className="mt-2 text-xs" style={{ color: "rgba(247,246,242,0.45)" }}>{CONTACT.address}</p>
      <p className="mt-1 text-xs" style={{ color: "rgba(247,246,242,0.45)" }}>{CONTACT.phone} · {CONTACT.email}</p>
      <p className="hy-mono mt-4 text-[10px]" style={{ color: "rgba(247,246,242,0.3)" }}>
...
export default function App() {
  ...
  return (
    <div className="hy-app mx-auto min-h-screen max-w-md" style={{ background: C.obsidian }}>
      <GlobalStyle />
      <TopBar />
